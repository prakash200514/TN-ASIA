...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $db->prepare("UPDATE lost_found SET status=? WHERE item_id=? AND depot_id=?")->execute([$status,$iid,$did]);
    redirectWithMessage(APP_URL.'/depot_manager/lost_found.php','success','Item status updated.');
}

$items=$db->query("SELECT lf.*,u.name reporter_name,b.bus_number FROM lost_found lf JOIN users u ON u.user_id=lf.reported_by LEFT JOIN buses b ON b.bus_id=lf.bus_id WHERE lf.depot_id=$did OR (lf.bus_id IN (SELECT bus_id FROM buses WHERE depot_id=$did)) ORDER BY lf.created_at DESC")->fetchAll();
...
<div class="main-content">
  <div class="topbar"><button class="sidebar-toggle" id="sidebarToggle"><i class="fa fa-bars"></i></button><div class="topbar-title">📦 Lost & Found</div></div>
  <div class="page-content">
    <?php if(!empty($flash['msg'])): ?><div class="flash-banner flash-<?=$flash['type']?> mb-3"><?=htmlspecialchars($flash['msg'])?></div><?php endif; ?>
    <div class="table-card">
      <div class="table-header"><h5><i class="fa fa-box-open me-2 text-success"></i>Lost & Found Items (<?=count($items)?>)</h5></div>
      <div class="table-responsive"><table class="data-table">
        <thead><tr><th>#</th><th>Item</th><th>Type</th><th>Reported By</th><th>Bus</th><th>Date</th><th>Status</th><th>Update</th></tr></thead>
        <tbody>
